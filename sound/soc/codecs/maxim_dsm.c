...
static inline int32_t maxdsm_dsm_open(void *data)
{
	...
	if (maxdsm.ignore_mask == MAXDSM_IGNORE_MASK_ALL &&
			...
			(maxdsm.filter_set == DSM_ID_FILTER_SET_AFE_R_CNTRLS)))
		return -EPERM;
		...
		unsigned int val, unsigned int flag)
{
	...
	case PLATFORM_TYPE_B:
		if (reg > maxdsm.param_size)
			...
			maxdsm_write_all();
			if (maxdsm.filter_set == DSM_ID_FILTER_SET_AFE_CNTRLS)
				maxdsm.filter_set = DSM_ID_FILTER_GET_AFE_PARAMS;
			else
				maxdsm.filter_set = DSM_ID_FILTER_GET_AFE_R_PARAMS;
			maxdsm_read_all();
...
void maxdsm_update_r_param(void)
{
	switch (maxdsm.platform_type) {
	case PLATFORM_TYPE_B:
		maxdsm.filter_set = DSM_ID_FILTER_GET_AFE_R_PARAMS;
...
EXPORT_SYMBOL_GPL(maxdsm_update_r_param);
...
int maxdsm_set_rdc_temp(int rdc, int rdc_r, int temp)
{
	int ret = 0;
	dbg_maxdsm("rdc=0x%08x rdc_r=0x%08x temp=0x%08x", rdc, rdc_r, temp);
	maxdsm.tx_port_id |= 1 << 31;
	maxdsm.param[PARAM_WRITE_FLAG] = FLAG_WRITE_RDC_CAL_ONLY;
	maxdsm.param[PARAM_VOICE_COIL] = rdc;
	maxdsm.param[PARAM_AMBIENT_TEMP] = temp << 19;
	maxdsm.filter_set = DSM_ID_FILTER_SET_AFE_CNTRLS;
	ret = maxdsm_dsm_open(&maxdsm);
	if (maxdsm_is_stereo() && rdc_r) {
		maxdsm.param[PARAM_VOICE_COIL] = rdc_r;
		maxdsm.filter_set = DSM_ID_FILTER_SET_AFE_R_CNTRLS;
		ret = maxdsm_dsm_open(&maxdsm);
	}
	maxdsm.tx_port_id &= ~(1 << 31);
...
int maxdsm_set_pilot_signal_r_state(int on)
{
	int ret = 0;
	...
	ret = maxdsm_dsm_open(&maxdsm);
	if (!(maxdsm.param[PARAM_FEATURE_SET] & 0x200)) {
		dbg_maxdsm("Feature set right param was not updated. 0x%08x",
				maxdsm.param[PARAM_FEATURE_SET]);
		ret = -EAGAIN;
	}
		...
		void __user *argp)
{
	...
	case MAXDSM_IOCTL_SET_PARAM_ID:
		if ((arg != DSM_ID_FILTER_GET_AFE_PARAMS) &&
				(arg != DSM_ID_FILTER_SET_AFE_CNTRLS) &&
				(arg != DSM_ID_FILTER_GET_AFE_R_PARAMS) &&
				(arg != DSM_ID_FILTER_SET_AFE_R_CNTRLS))
			goto error;
